...
def get_center_angle(
    ...
    coord = right_coord-left_coord
    # normalize x and y
    norm = np.linalg.norm(coord)
    coord = coord/norm
    x = coord[0]
    y = coord[1]
    # arctan2 returns in the range [-pi, pi], so need to transform it to [0, 180]
    angle_arctan2 = np.arctan2(y, x)
    angle = abs(angle_arctan2*180/np.pi)
    # ===============================================================================
...
class AugmentedDataset(Dataset):
    def __init__(self, rgb_dataset: RGBDataset):
        ...
        '''
        apply affine transformation on 70% of all images
        translation is defined to be:
    ...
    def __getitem__(self, idx: int
        ...
        angle = np.float32(data_torch['angle'])
        
        left_coord, right_coord = get_finger_points(center_point, angle)
        ...
        transformed_img, transformed_kps = self.aug_pipeline(image=rgb, keypoints=kps)
        
        left = transformed_kps[0].xy
        right = transformed_kps[1].xy
        center_trans, angle_trans = get_center_angle(left, right)

        data_torch['rgb'] = torch.from_numpy(transformed_img).to(torch.uint8)
        data_torch['center_point'] = center_trans
        data_torch['angle'] = angle_trans
        # ===============================================================================
        return data_torch
